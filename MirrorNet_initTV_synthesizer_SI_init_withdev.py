...
out = "figs/NEW_with_only_1_loss_for_SI_LTsyn_" + str(date.today()) + "_H_" + str(datetime.datetime.now().hour)
out_dir = "./figs/NEW_with_only_1_loss_for_SI_LTsyn_" + str(date.today()) + "_H_" + str(datetime.datetime.now().hour) + "/"
...
# setting path to new init trained synthesizer
model_dir = "./figs/NEW_with_only_1_loss_for_speech_DATE_syn_9TVs_2023-01-16_H_17/"
...
parser.add_argument('--lr', type=float, default=1e-3,
                    help='learning rate')
parser.add_argument('--decoder_lr', type=float, default=1e-6,  # changed from 3e-4
                    help='learning rate')
parser.add_argument('--encoder_lr', type=float, default=1e-6,  # changed from 3e-4
                    help='learning rate')
...
# Initialize optimizers for initialization stage

E_optimizer = optim.Adam(E.parameters(), lr=1e-3)
...
#
trainTogether_newTechnique(epochs=110, init=True, train_D=True, train_E=True, loader_eval="train", save_better_model=False)
...
# Reinitialize optimizers for training phase

current_lr = args.lr
E_optimizer = optim.Adam(E.parameters(), lr=args.encoder_lr)
E_scheduler = torch.optim.lr_scheduler.ExponentialLR(E_optimizer, gamma=0.7)  # changed gamma=0.5 to 0.3
# E_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(E_optimizer, mode='min', factor=0.5, patience=20, verbose=True, threshold=1e-7)
